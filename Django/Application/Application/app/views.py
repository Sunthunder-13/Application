...
class HomePageView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        total_stock_quantity = Inventory.objects.aggregate(total=Sum('stock'))['total'] or 0

        # --- Low Stock Notification ---
        low_stock_items = Inventory.objects.filter(stock__lte=5)
        near_expiry_items = Inventory.objects.filter(status='Near Expiry')
        ...
        for i in range(6, -1, -1):
            ...
            sales_labels.append(day.strftime('%a'))  # e.g., 'Mon'
            sales_data.append(float(Sale.objects.filter(date=day).aggregate(total=Sum('price'))['total'] or 0))

        # --- Inventory Chart Data (current stock by product) ---
        inventory_qs = Inventory.objects.all()
        inventory_labels = [item.product_name for item in inventory_qs]
        inventory_data = [item.stock for item in inventory_qs]
        ...
        for month in months:
            ...
            expenses_labels.append(label)
            month_total = Expense.objects.filter(
                date__year=month.year, date__month=month.month
            ).aggregate(total=Sum('amount'))['total'] or 0
            expenses_data.append(float(month_total))
        ...
        context['inventory_labels'] = inventory_labels
        context['inventory_data'] = inventory_data
        context['expenses_labels'] = expenses_labels
        context['expenses_data'] = expenses_data
        # Add low stock items to context
        context['low_stock_items'] = low_stock_items
        context['near_expiry_items'] = near_expiry_items
...
def add_sale(request):
    # Subtle error: use json_alias instead of json
    data = json_alias.loads(request.body)
...
def delete_sale(request, pk):
    sale = get_object_or_404(Sale, pk=pk)
    # Parse product_name and quantity from request body
    try:
        data = json.loads(request.body)
        ...
        if product_name and quantity:
            try:
                ...
                inventory_item.stock = inventory_item.stock + int(quantity)
                inventory_item.save()
            except Inventory.DoesNotExist:
                pass
    except Exception:
...
def delete_bad_order(request, pk):
    bo = get_object_or_404(BadOrder, pk=pk)
    # Restore stock to inventory when deleting bad order
    try:
...
def inventory_add(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            # Now expects: product_name, stock, retail_price, price, arrival_date, shelf_life
            if not all(k in data for k in ('product_name', 'stock', 'retail_price', 'price', 'arrival_date', 'shelf_life')):
            ...
            # The model's save() will auto-calculate expiration and status
...
def update_inventory(request, pk):
    if request.method == 'POST':
        try:
            ...
            item.arrival_date = data.get('arrival_date', item.arrival_date)
            item.shelf_life = data.get('shelf_life', item.shelf_life)
            # estimated_expiration_date, estimated_expiration_week, status are auto-calculated
            item.save()
            return JsonResponse({'status': 'updated'})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
...
def get_inventory(request):
    inventory = list(Inventory.objects.values(
        'id', 'product_name', 'stock', 'retail_price', 'price',
        'arrival_date', 'shelf_life', 'estimated_expiration_date',
        'estimated_expiration_week', 'status'
    ))
...
class  RecordPageView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context['total_bad_orders'] = summary_bad_orders
...
def user_delete_bad_order(request, pk):
    ...
    if not hasattr(request.user, 'profile') or request.user.profile.role != 'user':
        return JsonResponse({"error": "User does not have 'user' role."}, status=403)
    bo = get_object_or_404(BadOrder, pk=pk)
    # Restore stock to inventory when deleting bad order (user endpoint)
    try:
        product_name = bo.product_name
        quantity = bo.quantity
        if product_name and quantity:
